 ...
 *
 * 評価基準（0-10点）：
 * - S級（8.0+）：プロトレーダー - 完璧に近いパフォーマンス
 * - A級（6.5-7.9）：上級トレーダー - 優秀で安定した成績
 * - B級（5.0-6.4）：中級トレーダー - 基本を習得、改善の余地あり
 * - C級（3.5-4.9）：初級トレーダー - 発展途上、学習段階
 * - D級（-3.5）：入門トレーダー - 基礎習得が必要
 */
function getLevel(overallScore: number): string {
  if (overallScore >= 8.0) return 'S級：プロトレーダー';
  if (overallScore >= 6.5) return 'A級：上級トレーダー';
  if (overallScore >= 5.0) return 'B級：中級トレーダー';
  if (overallScore >= 3.5) return 'C級：初級トレーダー';
  return 'D級：入門トレーダー';
}
