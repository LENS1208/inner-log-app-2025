...
function getVolatility(trade: Trade): string {
  const pips = Math.abs(trade.pips || 0);
  if (pips > 100) return '高ボラ';
  if (pips > 30) return '中ボラ';
  return '低ボラ';
}

export default function SetupDetailDrawer({
  isOpen,
  onClose,
  setupTag,
  trades,
  ...
  const volatilityStats = useMemo(() => {
    const map = new Map<string, { profit: number; count: number }>();
    const order = ['高ボラ', '中ボラ', '低ボラ'];

    trades.forEach(t => {
      const vol = getVolatility(t);
      const current = map.get(vol) || { profit: 0, count: 0 };
      map.set(vol, {
        profit: current.profit + getTradeProfit(t),
        count: current.count + 1
      });
    ...
    return order.map(vol => ({
      vol,
      profit: map.get(vol)?.profit || 0,
      count: map.get(vol)?.count || 0
    }));
  }, [trades]);
  ...
  const aiComment = useMemo(() => {
    ...
    if (stats.pf > 1.5) {
      comment += 'PFが優秀で収益性が高い戦略です。';
    } else if (stats.pf < 1.0) {
      comment += 'PFが1.0未満で改善が必要です。';
    } else {
      comment += 'バランスの取れた戦略です。';
    }
    ...
    if (topTimeSlot && topTimeSlot.count > trades.length * 0.3) {
      comment += `${topTimeSlot.slot}に集中しています。`;
    }

    const topPair = pairStats[0];
    if (topPair && topPair.count > trades.length * 0.4) {
      comment += `${topPair.pair}での取引が中心です。`;
    }

    return comment;
  }, [setupTag, stats, timeOfDayStats, pairStats, trades.length]);
      ...
      >
        <div style={{ padding: 24 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>戦略構造</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
              ...
              {pairStats.length > 0 && (
                <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
                  <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>通貨ペア別損益（上位3）</h4>
                  <div style={{ height: 150 }}>
                    <Bar
                      data={{
              ...
              {/* 相場状態別損益 */}
              <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
                <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>相場状態別損益</h4>
                <div style={{ height: 150 }}>
                  <Bar
                    data={{
                      labels: volatilityStats.map(v => v.vol),
                      datasets: [{
                        label: '損益',
                        data: volatilityStats.map(v => v.profit),
                        backgroundColor: volatilityStats.map(v => v.profit >= 0 ? getAccentColor() : getLossColor()),
                      }],
                    }}
                    options={{
                      responsive: true,
          ...
          {/* 方向・行動のクセ */}
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>方向・行動のクセ</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              {/* 売り vs 買い */}
              {(directionStats.long.count + directionStats.short.count > 0) && (
                <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
                  <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>売り vs 買い</h4>
                  <div style={{ height: 150 }}>
                    <Pie
                      data={{
                        labels: ['買い', '売り'],
