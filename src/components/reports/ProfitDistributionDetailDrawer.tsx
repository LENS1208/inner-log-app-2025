...
interface ProfitDistributionDetailDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  rangeLabel: string;
  minProfit: number;
...
export default function ProfitDistributionDetailDrawer({ isOpen, onClose, rangeLabel, minProfit, maxProfit, trades }: ProfitDistributionDetailDrawerProps) {
  ...
  const setupStats = useMemo(() => {
    ...
    trades.forEach(t => {
      ...
      const current = map.get(setup) || { count: 0 };
      map.set(setup, {
        count: current.count + 1
      });
    });
  ...
  const aiComment = useMemo(() => {
    ...
    if (topTimeSlot && topTimeSlot.count > trades.length * 0.3) {
      comment += `の${topTimeSlot.slot}`;
    }

    if (topSetup) {
      comment += `${topSetup.setup}`;
    }

    comment += 'で発生しており、';

    if (longHoldTrades.length > trades.length * 0.5) {
      comment += '保有時間が1時間を超えたポジションに集中しています。';
    } else {
      comment += '比較的短期間で決済される傾向があります。';
    }

    return comment;
  }, [rangeLabel, minProfit, pairStats, setupStats, timeOfDayStats, trades]);
    ...
    <>
      ...
      >
        <div style={{ padding: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
              ...
              {pairStats.length > 0 && (
                <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
                  <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>通貨ペア別件数</h4>
          ...
          {/* 時間・保有時間との関係 */}
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>時間・保有時間との関係</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
              {/* 時間帯別件数 */}
              <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
