...
interface MarketConditionDetailDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  condition: string;
  trades: Trade[];
...
export default function MarketConditionDetailDrawer({ isOpen, onClose, condition, trades }: MarketConditionDetailDrawerProps) {
  useEffect(() => {
  ...
  const aiComment = useMemo(() => {
    ...
    if (condition.includes('高')) {
      comment += '高ボラ環境では';
    } else if (condition.includes('中')) {
      comment += '中ボラ環境では';
    } else if (condition.includes('低')) {
      comment += '低ボラ環境では';
    } else {
      comment += 'この相場状態では';
    }
    ...
    if (bestSetup && bestSetup.profit > 0) {
      comment += `${bestSetup.setup}戦略が最も機能し、`;
      if (topPair) {
        comment += `${topPair.pair}を中心に`;
      }
      comment += '順張りが成功要因になっています。';
    } else {
      comment += '全体的に苦戦しており、戦略の見直しが必要です。';
    }

    if (worstTimeSlot && worstTimeSlot.profit < 0) {
      comment += ` 一方で${worstTimeSlot.slot}の`;
      if (worstSetup && worstSetup.profit < 0) {
        comment += `${worstSetup.setup}では`;
      }
      comment += '損失が集中しています。';
    }

    return comment;
  }, [condition, setupStats, timeOfDayStats, pairStats]);
      ...
      >
        <div style={{ padding: 24 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>環境×構造分析</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
              ...
              {setupStats.length > 0 && (
                <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
                  <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>戦略タグ別損益</h4>
                  <div style={{ height: 150 }}>
                    <Bar
                      data={{
          ...
          {/* 相場状態 × 時間帯 の相関 */}
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>相場状態 × 時間帯 の相関</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
              {/* 時間帯別損益 */}
              <div style={{ background: 'var(--chip)', padding: 16, borderRadius: 8 }}>
