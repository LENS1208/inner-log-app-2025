...
interface DirectionDetailDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  direction: 'BUY' | 'SELL';
  trades: Trade[];
...
export default function DirectionDetailDrawer({
  isOpen,
  onClose,
  direction,
  trades,
  ...
  const aiComment = useMemo(() => {
    let comment = `${directionLabel}ポジションは`;
    ...
    if (topSetup && topSetup.count > trades.length * 0.3) {
      comment += `${topSetup.setup}戦略との相性が良いです。`;
    }

    return comment;
  }, [directionLabel, stats, pairStats, setupStats, trades.length]);
